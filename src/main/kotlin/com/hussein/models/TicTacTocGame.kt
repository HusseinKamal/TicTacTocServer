...
class TicTacTocGame{
    ...
    fun connectPlayer(session: WebSocketSession): Char? {
        ...
        state.update {
            ...
            it.copy(
                connectedPlayers = it.connectedPlayers + player //This to add 2 players to list in same connection
            )
        }
        return player
    ...
    fun disconnectPlayer(player:Char){
        ...
        state.update {
            it.copy(
                connectedPlayers = it.connectedPlayers - player //This to remove player from list of same connection
            )
        }
    }
    ...
    fun finishTurn(player: Char , x:Int ,y:Int){
        ...
        state.update {
            ...
            val isBoard = newField.all{it.all{it!= null}}
            if(isBoard){
            ...
            it.copy(
                ...
                field = newField,
                isBoardFull = isBoard,
                winningPlayer = getWinningPlayeer()?.also{
                    startNewRoundDelayed()
                }
            )
        }
    ...
    private fun getWinningPlayeer(): Char? {
